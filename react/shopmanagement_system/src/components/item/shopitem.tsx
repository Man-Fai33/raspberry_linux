...
export default function ShopItem(props: {
    ...
    return (
        <>
            <div className="w-full  h-80 flex flex-col   hover:scale-110 bg-slate-50 rounded-lg hover:border-red-600 hover:border-solid hover:border-2" onClick={() => { navigate('/shop/item', { state: props.data }); }}>
                <div className="relative w-full  h-2/3 rounded-lg " >
                    <img src={photo[0]} key={_id} loading="lazy" alt="" className=" h-full w-full   bg-center bg-cover bg-fixed rounded-sm" />
                    <div className="absolute right-1 top-1  bg-amber-200 rounded-md    w-16"> <div className=" flex justify-center"> <DiscountIcon sx={{ width: '14px' }} /> {FuncHelper.CountOfOne(discount) * 10}折</div></div>
